...
/* So this class is the DBScanKD class, in it you're going to find all the DBScan methods that are originally implemented 
...
public class DBScanKD{
    ...
    NearestNeighborsKD nb; // we initialize a new Neighbor instance that we're going to use later on
    Stack<Point3D> S = new Stack<Point3D>(); // the stack we will use in the method findcluster
    
    public DBScanKD(List<Point3D> DB){ // the constructor where we initialize DB and C
        this.DB = DB; 
    ...
    public void findClusters(){ 
        ...
        nb = new NearestNeighborsKD(DB); //we give the Neighbor instance that we initialized earlier DB as a parameter
